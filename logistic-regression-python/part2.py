...
def logisticRegression_L1(X, y, rate, reg):
    N,d = np.shape(X)
    w = np.zeros(d)
    #loss_history = []
    ...
    for i in range(0,10000): #10000 max iterations
        ...
        sums = np.dot((y - sig), X)
        w = w + (rate/N)*sums #normal gradient without L1 norm
        
        #L1 norm contribution
        w_sign = np.sign(w)
        thresholding = np.maximum(np.abs(w) - rate*reg, 0)
        w = np.multiply(w_sign, thresholding)
...
def testRegularization(trainX, trainY, validX, validY, rate):
    ...
    for i in range(0,6):
        ...
        w = logisticRegression_L1(trainX, trainY, rate, reg)
    ...
    ax.set_title('LASSO logistic regression accuracies')
    ax.set_ylim([0,100])
    ax.set_xticks(ind)
